...
class DashboardManager:
    ...
    def get_skill_distribution(self):
        ...
        cursor.execute("""
            WITH RECURSIVE split(skill, rest) AS (
                ...
                UNION ALL
                SELECT
                    substring(rest FROM 1 FOR strpos(rest, ',') - 1),
                    substring(rest FROM strpos(rest, ',') + 1)
                FROM split
                WHERE rest <> ''
            ),
            SkillCategories AS (
    ...
    def get_detailed_insights(self):
        """
        Get detailed insights from the database (PostgreSQL-compatible)
        """
        cursor = self.conn.cursor()
        insights = []
        ...
        cursor.execute("""
            SELECT rd.target_category, AVG(ra.ats_score) as avg_score,
        ...
        # Recent Improvement (weekly trend)
        cursor.execute("""
            SELECT 
                (SELECT AVG(ats_score) FROM resume_analysis 
                WHERE created_at >= CURRENT_DATE - INTERVAL '7 days') as recent_score,
                (SELECT AVG(ats_score) FROM resume_analysis 
                WHERE created_at < CURRENT_DATE - INTERVAL '7 days') as old_score
        """)
        ...
        cursor.execute("""
            ...
            FROM (
                SELECT unnest(string_to_array(skills, ','))::text AS skill
                FROM resume_data
        ...
        if top_skills:
            skills_text = ", ".join([f"{s[0].strip()} ({s[1]} resumes)" for s in top_skills])
            insights.append({
                "title": "Top Skills",
                "icon": "💡",
